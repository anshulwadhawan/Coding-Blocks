...
function search(el){
    ...
    var input2='.ip2';
    var textbox=$(input1);
    var obj=JSON.parse(localStorage.getItem('GIFs'));
...
function addtofavs(el){
    ...
    if(localStorage.getItem("GIFs")!==null) {
        obj = JSON.parse(localStorage.getItem("GIFs"));
    }
    else
        obj={};
    ...
    // console.log(el.parentElement.parentElement.parentElement.firstChild.firstChild.nextSibling.src);
    obj[el.name].push(el.parentElement.parentElement.parentElement.firstChild.firstChild.nextSibling.src);
    localStorage.setItem('GIFs',JSON.stringify(obj));
    console.log(JSON.parse(localStorage.getItem("GIFs")));
